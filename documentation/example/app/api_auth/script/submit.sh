...
# This gets rejected
curl -X POST -H "Authorization: FakeTokenNotGonnaWork" -H 'Content-Type: application/json' -d '{"items": [{"test1": 1}, {"test2": 2}]}' -w '\n\n' ${API_URL}/prod/auth-cognito

curl -X POST -H "Authorization: ${TOKEN}" -H 'Content-Type: application/json' -d '{"items": [{"test1": 1}, {"test2": 2}]}' -w '\n\n' ${API_URL}/prod/auth-cognito
