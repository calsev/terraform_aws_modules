...
locals {
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      ...
      image_environment_key_prefix    = v.image_environment_key_prefix == null ? var.build_image_environment_key_prefix_default : v.image_environment_key_prefix
      image_environment_variable_list = v.image_environment_variable_list == null ? var.build_image_environment_variable_list_default : v.image_environment_variable_list
  ...
  l2_map = {
    for k, v in local.l0_map : k => {
      image_environment_key_arch     = "${local.l1_map[k].image_environment_key_prefix}${v.image_environment_key_arch == null ? var.build_image_environment_key_arch_default : v.image_environment_key_arch}"
      image_environment_key_repo_url = "${local.l1_map[k].image_environment_key_prefix}${v.image_environment_key_repo_url == null ? var.build_image_environment_key_repo_url_default : v.image_environment_key_repo_url}"
      image_environment_key_tag      = "${local.l1_map[k].image_environment_key_prefix}${v.image_environment_key_tag == null ? var.build_image_environment_key_tag_default : v.image_environment_key_tag}"
    }
  }
  l3_map = {
    for k, v in local.l0_map : k => {
      environment_variable_map_repo_url = local.l1_map[k].image_ecr_repo_key == null ? null : {
        (local.l2_map[k].image_environment_key_repo_url) = {
          type  = "PLAINTEXT"
          value = var.ecr_data_map[local.l1_map[k].image_ecr_repo_key].repo_url
        }
      }
    }
  }
        ...
        contains(local.l1_map[k].image_build_arch_list, "amd") ? {
          pipe_image_amd = merge(local.l1_map[k], local.repo_build_image_common, {
            ...
            environment_variable_map = merge(
              {
                (local.l2_map[k].image_environment_key_arch) = {
                  type  = "PLAINTEXT"
                  value = "linux/amd64"
                }
                (local.l2_map[k].image_environment_key_tag) = {
                  type  = "PLAINTEXT"
                  value = "${local.l1_map[k].image_tag_base}-amd"
                }
              },
              local.l3_map[k].environment_variable_map_repo_url,
            )
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      {
        for k_attr, v_attr in v : k_attr => v_attr if !contains(["environment_variable_map_repo_url"], k_attr)
      },
        ...
        build_stage_list = [
          {
            action_map = {
              for arch in v.image_build_arch_list : title(arch) => {
                configuration_build_environment_map = {
                  for v in local.l1_map[k].image_environment_variable_list : v => {
                    type  = "PLAINTEXT"
                    value = "#{variables.${v}}"
                  }
                }
