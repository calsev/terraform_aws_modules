...
locals {
  ...
  create_alias_map = {
    for k, v in local.lx_map : k => merge(v, {
      dns_alias_name    = aws_s3_bucket_website_configuration.this_web_config[k].website_endpoint # The s3 website uses the endpoint, but the alias uses the domain and sni
