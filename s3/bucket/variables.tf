...
variable "bucket_lifecycle_version_expiration_days_default" {
  type        = number
  default     = 183
  description = "Both version count and days must be satisfied to expire an object. Ignored if greater than lifecycle_expiration_days."
}
...
variable "dns_data" {
  ...
  default     = null
  description = "Must be provided if any bucket has DNS enabled"
}
