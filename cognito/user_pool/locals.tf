...
locals {
  ...
  create_domain_map = {
    for k, v in local.lx_map : k => v if v.dns_from_zone_key != null
  }
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      ...
      sign_in_allowed_first_auth_factor_list            = v.sign_in_allowed_first_auth_factor_list == null ? var.pool_sign_in_allowed_first_auth_factor_list_default : v.sign_in_allowed_first_auth_factor_list
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      ...
      },
      {
        ...
        user_pool_id            = aws_cognito_user_pool.this_pool[k].id
      }
    )
  }
}
