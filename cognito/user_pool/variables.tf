...
variable "pool_map" {
  type = map(object({
    ...
    email_from_display_name               = optional(string)
    ...
    lambda_arn_custom_email_sender            = optional(string)
    lambda_arn_custom_message                 = optional(string)
    lambda_arn_custom_sms_sender              = optional(string)
    ...
    sign_in_allowed_first_auth_factor_list = optional(list(string))
...
variable "pool_email_from_address_default" {
  type    = string
  default = null
}

variable "pool_email_from_display_name_default" {
  type    = string
  default = null
}
...
variable "pool_lambda_arn_custom_email_sender_default" {
  type    = string
  default = null
}
...
variable "pool_lambda_arn_custom_sms_sender_default" {
  type    = string
  default = null
}
...
variable "pool_lambda_kms_key_arn_default" {
  type    = string
  default = null
}
...
variable "pool_sign_in_allowed_first_auth_factor_list_default" {
  type = list(string)
  default = [
    "PASSWORD",
    "WEB_AUTHN",
  ...
  validation {
    condition     = length(setsubtract(toset(var.pool_sign_in_allowed_first_auth_factor_list_default), toset(["EMAIL_OTP", "PASSWORD", "SMS_OTP", "WEB_AUTHN"]))) == 0
    error_message = "Invalid first factors"
  }
}
...
variable "pool_web_auth_user_verification_required_default" {
  type        = bool
  default     = true
  description = "If true, user verification for web authentication (passkey) is required, rather than preferred"
}

variable "ses_data_map" {
  type = map(object({
