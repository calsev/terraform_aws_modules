...
variable "domain_map" {
  type = map(object({
    ...
    dmarc_record_string         = optional(string)
...
variable "domain_dmarc_record_string_default" {
  type        = string
  default     = null
  description = "This should be something like 'v=DMARC1; p=reject; rua=mailto:dmarc@domain.com; ruf=mailto:dmarc@domain.com; sp=none; aspf=r; adkim=s; fo=1;'"
}
