...
locals {
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], module.alarm_map.data[k], module.vpc_map.data[k], {
      ...
      log_db_key                                  = v.log_db_key == null ? var.elb_log_db_key_default : v.log_db_key
  ...
  l2_map = {
    for k, v in local.l0_map : k => {
      ...
      log_db_enabled                                               = local.l1_map[k].log_db_key != null
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      ...
      local.elb_data_map_emulated[k],
      {
        ...
        log_db                     = v.log_db_enabled ? module.log_db.data[v.log_db_key] : null
        port_to_protocol_to_listener_map = {
