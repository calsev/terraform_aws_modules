locals {
  arn_list_application_inference_profile = [
    for name in sort(distinct(var.name_list_application_inference_profile)) : startswith(name, "arn:") ? name : "arn:${var.std_map.iam_partition}:bedrock:${var.std_map.aws_region_name}:${var.std_map.aws_account_id}:application-inference-profile/${name}"
  ]
  ...
  arn_list_inference_profile = [
    for name in sort(distinct(var.name_list_inference_profile)) : startswith(name, "arn:") ? name : "arn:${var.std_map.iam_partition}:bedrock:${var.std_map.aws_region_name}:${var.std_map.aws_account_id}:inference-profile/${name}"
  ]
