      ...
      },
      {
        policy_map = {
          for k_access, v_access in v.sid_map : k_access => merge(
            ...
            {
              iam_policy_doc = jsondecode(local.create_policy_map[v_access.k_all].iam_policy_json)
              iam_policy_arn = v.policy_create ? aws_iam_policy.policy[v_access.k_all].arn : null
            },
          )
        }
      }
