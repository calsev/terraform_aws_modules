locals {
  ...
  policy_name           = local.name_sanitized == null ? null : var.name_infix ? "${local.name_prefix_sanitized}${local.name_context}" : "${local.name_prefix_sanitized}${local.name_sanitized}"
