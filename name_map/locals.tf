locals {
  l1_map = {
    for k, v in var.name_map : k => merge(v, {
      name_append                     = lower(replace(replace(v.name_append == null ? var.name_append_default : v.name_append, local.name_regex_prefix, "-"), "--", "-"))
      ...
      name_prepend                    = lower(replace(replace(v.name_prepend == null ? var.name_prepend_default : v.name_prepend, local.name_regex_prefix, "-"), "--", "-"))
