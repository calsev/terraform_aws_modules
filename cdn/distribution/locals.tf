...
locals {
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      ...
      error_response_map                                    = v.error_response_map == null ? var.domain_error_response_map_default : v.error_response_map
  ...
  l3_map = {
    for k, v in var.domain_map : k => {
      ...
      response_cors_allowed_origin_list_all = local.l2_map[k].response_cors_allowed_origin_list == ["*"] ? local.l2_map[k].response_cors_allowed_origin_list : concat([for fqdn in local.l2_map[k].alias_name_list : "https://${fqdn}"], local.l2_map[k].response_cors_allowed_origin_list) # Include aliases even without DNS enabled
