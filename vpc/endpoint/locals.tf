...
locals {
  create_association_1_list = flatten([
    for k, v in local.lx_map : [
      for r_id in v.vpc_route_table_id_list : merge(v, {
        route_table_id = r_id
        k_all          = "${k}_${r_id}"
        k_end          = k
      })
  ...
  create_association_x_map = {
    for v in local.create_association_1_list : v.k_all => v
  }
  l0_list = flatten([
    for k, v in var.vpc_map : [
      for k_end, v_end in v.endpoint_map == null ? merge(var.endpoint_map_free_default, var.endpoint_map_standard_default, var.endpoint_map_custom_default) : v.endpoint_map : merge(v, v_end, {
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      ...
      dns_enabled                 = v.dns_enabled == null ? var.endpoint_dns_enabled_default : v.dns_enabled
  ...
  l2_map = {
    for k, v in local.l0_map : k => {
      ...
      private_dns_enabled        = local.l1_map[k].endpoint_type == "Gateway" ? false : v.private_dns_enabled == null ? var.endpoint_private_dns_enabled_default : v.private_dns_enabled
      ...
      vpc_security_group_id_list = local.l1_map[k].endpoint_type == "Gateway" ? [] : [for k_sg in local.l1_map[k].vpc_security_group_key_list : v.security_group_id_map[k_sg]]
    }
  }
  l3_map = {
    for k, v in local.l0_map : k => {
      private_dns_for_inbound_resolver_endpoint_only = local.l2_map[k].private_dns_enabled ? v.private_dns_for_inbound_resolver_endpoint_only == null ? var.endpoint_private_dns_for_inbound_resolver_endpoint_only_default : v.private_dns_for_inbound_resolver_endpoint_only : null
      ...
      vpc_subnet_id_eni_list                         = local.l1_map[k].endpoint_type == "Gateway" ? [] : [for _, v_az in v.segment_map[local.l1_map[k].endpoint_segment_key].subnet_map : v_az.subnet_id]
    }
  }
  l4_map = {
    for k, v in local.l0_map : k => {
  ...
  output_data = {
    for k, v in var.vpc_map : k => merge(v, {
      endpoint_map = {
        for k_end, v_end in v.endpoint_map == null ? merge(var.endpoint_map_free_default, var.endpoint_map_standard_default, var.endpoint_map_custom_default) : v.endpoint_map : k_end => merge(
          {
