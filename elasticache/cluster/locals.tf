...
locals {
  ...
  create_cluster_x_map = {
    for k, v in local.create_cluster_1_map : k => v if !v.is_replication_group
  }
  create_group_x_map = {
    for k, v in local.create_cluster_1_map : k => v if v.is_replication_group
  }
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], module.vpc_map.data[k], {
      ...
      cluster_mode                       = v.cluster_mode == null ? var.cluster_mode_default : v.cluster_mode
      data_tiering_enabled               = v.data_tiering_enabled == null ? var.cluster_data_tiering_enabled_default : v.data_tiering_enabled
      encryption_at_rest_enabled         = v.encryption_at_rest_enabled == null ? var.cluster_encryption_at_rest_enabled_default : v.encryption_at_rest_enabled
      ...
      kms_key_id                         = v.kms_key_id == null ? var.cluster_kms_key_id_default : v.kms_key_id
      ...
      shard_count                        = v.shard_count == null ? var.cluster_shard_count_default : v.shard_count
      shard_replica_count                = v.shard_replica_count == null ? var.cluster_shard_replica_count_default : v.shard_replica_count
      ...
      transit_encryption_required        = v.transit_encryption_required == null ? var.cluster_transit_encryption_required_default : v.transit_encryption_required
    })
  }
  l2_map = {
    for k, v in local.l0_map : k => {
      engine_version            = v.engine_version == null ? var.cluster_engine_version_default_map[local.l1_map[k].engine] : v.engine_version
      final_snapshot_identifier = local.l1_map[k].final_snapshot_enabled ? local.l1_map[k].name_effective : null
      ip_discovery_mode         = local.l1_map[k].shard_count > 1 ? local.l1_map[k].ip_discovery_v6 ? "ipv6" : "ipv4" : null
      is_replication_group = (
        local.l1_map[k].engine == "valkey" ||
        local.l1_map[k].engine == "redis" && (
          local.l1_map[k].shard_count > 1 ||
          local.l1_map[k].shard_replica_count > 0
        )
      )
      log_map                              = local.l1_map[k].engine == "memcached" ? {} : v.log_map == null ? var.cluster_log_map_default : v.log_map
      multi_az_enabled                     = local.l1_map[k].shard_replica_count > 0 || local.l1_map[k].engine == "memcached" ? v.multi_az_enabled == null ? var.cluster_multi_az_enabled_default : v.multi_az_enabled : false
      ...
      parameter_group_name                 = v.parameter_group_name == null ? var.cluster_parameter_group_name_default_map[local.l1_map[k].engine] : v.parameter_group_name
      port                                 = v.port == null ? var.cluster_port_default_map[local.l1_map[k].engine] : v.port
  ...
  l3_map = {
    for k, v in local.l0_map : k => {
      automatic_failover_enabled = local.l1_map[k].shard_replica_count > 0 || local.l2_map[k].multi_az_enabled # There is not reason not to fail over with a replica, and it is required for multi-az
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      ...
      {
        cluster_arn                      = v.is_replication_group ? aws_elasticache_replication_group.this_cluster[k].arn : aws_elasticache_cluster.this_cluster[k].arn
        cluster_cache_node_list          = v.is_replication_group ? aws_elasticache_replication_group.this_cluster[k].member_clusters : aws_elasticache_cluster.this_cluster[k].cache_nodes
        cluster_primary_endpoint_address = v.is_replication_group ? aws_elasticache_replication_group.this_cluster[k].primary_endpoint_address : null
        cluster_reader_endpoint_address  = v.is_replication_group ? aws_elasticache_replication_group.this_cluster[k].reader_endpoint_address : null
