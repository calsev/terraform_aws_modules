...
locals {
  ...
  db_map = {
    for k, v in var.db_map : k => merge(
      ...
      local.performance_insights_map[k],
      {
        ...
        iam_role_arn_monitoring               = v.iam_role_arn_monitoring == null ? var.db_iam_role_arn_monitoring_default : v.iam_role_arn_monitoring
        ...
        #ignore_change_list = [
        #  for k_attrib, _ in local.ignore_change_map[k] : k_attrib
        #]
        instance_class               = v.instance_class == null ? var.db_instance_class_default : v.instance_class
        instance_identifier_prefix   = v.instance_identifier_prefix == null ? var.db_instance_identifier_prefix_default : v.instance_identifier_prefix
        ...
        password                     = v.password == null ? var.db_password_default : v.password
        performance_insights_enabled = local.performance_insights_map[k].performance_insights_kms_key_arn != null || !contains([0, null], local.performance_insights_map[k].performance_insights_retention_period_day)
        port                         = v.port == null ? var.db_port_default : v.port
        provisioned_iops             = local.iops_map[k]
        ...
        security_group_name_map = {
          for sg_name in local.security_group_name_map[k] : sg_name => var.sg_map[sg_name].id
        }
        skip_final_snapshot = v.final_snapshot_identifier == null
  ...
  #    },
  #    local.allow_upgrade_map[k].allow_major_version_upgrade || local.allow_upgrade_map[k].auto_minor_version_upgrade ? { engine_version = null } : {}
  #  )
  #}
  iops_map = {
  ...
  output_data = {
    for k, v in local.db_map : k => merge(v, {
      ...
      arn     = aws_db_instance.this_db[k].arn
      id      = aws_db_instance.this_db[k].id
    })
  }
  performance_insights_map = {
    for k, v in var.db_map : k => merge(v, {
