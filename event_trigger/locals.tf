locals {
  ...
  l2_map = {
    for k, v in var.event_map : k => {
      batch_targets  = local.l1_map[k].definition_arn == null ? {} : startswith(local.l1_map[k].definition_arn, "arn:${var.std_map.iam_partition}:batch:") ? { this = {} } : {}
      ecs_targets    = local.l1_map[k].definition_arn == null ? {} : startswith(local.l1_map[k].definition_arn, "arn:${var.std_map.iam_partition}:ecs:") ? { this = {} } : {}
      ...
      target_is_log = local.l1_map[k].target_arn == null ? false : startswith(local.l1_map[k].target_arn, "arn:${var.std_map.iam_partition}:logs:")
    }
  }
  l3_map = {
    for k, v in var.event_map : k => {
      ...
      iam_role_has_default       = local.l1_map[k].iam_role_arn_custom == null && !local.l2_map[k].target_is_log
