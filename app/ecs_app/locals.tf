    ...
    for k, v in local.l0_map : k => {
      ...
      elb_target_map = {
        for k_targ, v_targ in local.l1_map[k].elb_target_map : k_targ => merge(v_targ, {
          listen_protocol = v_targ.listen_protocol == null ? var.listener_listen_protocol_default : v_targ.listen_protocol
        })
      }
  ...
  l4_map = {
    for k, v in local.l0_map : k => {
      deployment_environment_map = merge(
        ...
        {
          blue = {
            listen_port = v.deployment_listen_port_prod == null ? var.app_deployment_listen_port_prod_default : v.deployment_listen_port_prod
          }
        },
        local.l3_map[k].deployment_style_use_blue_green ? {
          green = {
            listen_port = v.deployment_listen_port_test == null ? var.app_deployment_listen_port_test_default : v.deployment_listen_port_test
          }
        } : {}
      )
      elb_target_map = {
        # These keys must match the unrolled target keys, even if a single-keyed target uses blue-green
        for k_targ, v_targ in local.l2_map[k].elb_target_map : local.l3_map[k].deployment_style_use_blue_green ? "${k}_blue" : "${k}_${k_targ}" => v_targ
      }
    }
  }
  l5_map = {
    for k, v in local.l0_map : k => {
      listen_target_list = flatten([
        ...
        for k_targ, v_targ in local.l2_map[k].elb_target_map : [ # This is intentionally l2
    ...
    for k, v in local.l0_map : k => {
      listen_target_map = {
        for v_lt in local.l5_map[k].listen_target_list : v_lt.k_lt => merge(v_lt, {
          ...
          rule_condition_map = merge(v_lt.rule_condition_map == null ? var.rule_condition_map_default : v_lt.rule_condition_map, startswith(v_lt.listen_protocol, "HTTP") ? {
            host_match = {}
          } : {})
        })
      }
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      ...
      },
      {
        ...
        iam = {
          policy = {
            service_deploy = v.deployment_uses_codedeploy ? null : module.service_deploy_policy.data[k]
          }
          role = {
