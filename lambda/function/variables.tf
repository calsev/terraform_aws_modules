...
variable "function_map" {
  type = map(object({
    ...
    log_level_application                            = optional(string)
    log_level_system                                 = optional(string)
    ...
    source_package_created_archive_path              = optional(string) # Defaults to the directory with .zip appended or content file parent with package.zip appended
...
variable "function_log_format_default" {
  type    = string
  default = "Text"
  validation {
    condition     = contains(["JSON", "Text"], var.function_log_format_default)
    error_message = "Invalid log format"
  }
}

variable "function_log_level_application_default" {
  type        = string
  default     = "INFO"
  description = "Ignored unless log_format = JSON."
  validation {
    condition     = contains(["DEBUG", "ERROR", "FATAL", "INFO", "TRACE", "WARN"], var.function_log_level_application_default)
    error_message = "Invalid log level"
  }
}

variable "function_log_level_system_default" {
  type        = string
  default     = "INFO"
  description = "Ignored unless log_format = JSON."
  validation {
    condition     = contains(["DEBUG", "INFO", "WARN"], var.function_log_level_system_default)
    error_message = "Invalid log level"
  }
}
...
variable "function_source_kms_key_arn_default" {
  type    = string
  default = null
}
...
variable "function_tenant_isolation_mode_default" {
  type        = string
  default     = null
  description = "Lambda permissions not supported for functions with tenancy mode specified."
  validation {
    condition     = var.function_tenant_isolation_mode_default == null ? true : contains(["PER_TENANT"], var.function_tenant_isolation_mode_default)
    error_message = "Invalid log format"
  }
}
...
variable "function_tracing_mode_default" {
  ...
  default = "PassThrough"
  validation {
    condition     = contains(["Active", "PassThrough"], var.function_tracing_mode_default)
    error_message = "Invalid log format"
  }
}
