...
resource "aws_lambda_function" "this_function" {
  ...
  logging_config {
    application_log_level = each.value.log_level_application
    log_format            = each.value.log_format
    log_group             = each.value.log_group_name
    system_log_level      = each.value.log_level_system
  }
