...
locals {
  ...
  create_function_map = {
    for k, v in local.lx_map : k => merge(v, {
      log_group_name = module.log_group.data[k].log_group_name
    })
  }
  create_hash_map = {
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], module.vpc_map.data[k], {
      ...
      log_format                          = v.log_format == null ? var.function_log_format_default : v.log_format
      ...
      source_kms_key_arn                  = v.source_kms_key_arn == null ? var.function_source_kms_key_arn_default : v.source_kms_key_arn
      ...
      tenant_isolation_mode               = v.tenant_isolation_mode == null ? var.function_tenant_isolation_mode_default : v.tenant_isolation_mode
  ...
  l2_map = {
    for k, v in local.l0_map : k => {
      log_level_application             = local.l1_map[k].log_format == "JSON" ? v.log_level_application == null ? var.function_log_level_application_default : v.log_level_application : null
      log_level_system                  = local.l1_map[k].log_format == "JSON" ? v.log_level_system == null ? var.function_log_level_system_default : v.log_level_system : null
  ...
  l3_map = {
    for k, v in local.l0_map : k => {
      ...
      iam_policy_arn_source_image_read                 = local.l2_map[k].source_is_image ? var.ecr_data_map[local.l1_map[k].source_image_repo_key].policy_map["read"].iam_policy_arn : null
      ...
      source_image_uri                                 = local.l2_map[k].source_is_image ? "${var.ecr_data_map[local.l1_map[k].source_image_repo_key].repo_url}:${local.l1_map[k].source_image_repo_tag}" : null
  ...
  output_data = {
    for k, v in local.lx_map : k => merge(
      ...
      module.function_policy.data[k],
      {
        ...
        lambda_qualified_arn = aws_lambda_function.this_function[k].qualified_arn
        lambda_version       = aws_lambda_function.this_function[k].version
