module "name_map" {
  ...
  name_prefix_default = var.permission_name_prefix_default
  std_map             = var.std_map
}

locals {
  l0_map = {
    for k, v in var.permission_map : k => v if v.lambda_arn != null
  }
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      action     = v.action == null ? var.permission_action_default : v.action
      name       = split(":", v.lambda_arn)[length(split(":", v.lambda_arn)) - 1]
