...
if [ -x "$(command -v terraform)" ] && [[ "$(terraform --version)" =~ $tf_ver ]]; then
  ...
  aws s3 cp --no-progress "s3://cdn-bucket.calsev.com/installer/terraform_${sys}_${arch}_${tf_ver}" /usr/local/bin/terraform
...
if [ -x "$(command -v tflint)" ] && [[ "$(tflint --version)" =~ $tflint_ver ]]; then
  ...
  aws s3 cp --no-progress "s3://cdn-bucket.calsev.com/installer/tflint_${sys}_${arch}_${tflint_ver}" /usr/local/bin/tflint
