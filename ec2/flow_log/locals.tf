...
locals {
  ...
  create_db_table_map = {
    for k, v in local.lx_map : k => merge(v, {
      ...
      ${join(",\n", [for field in v.log_table_field_list : "  ${field.name} ${field.type}"])}
      )
  ...
  l1_map = {
    for k, v in local.l0_map : k => merge(v, module.name_map.data[k], {
      ...
      log_field_list                                 = v.log_field_list == null ? var.log_field_list_default : v.log_field_list
  ...
  l2_map = {
    for k, v in local.l0_map : k => {
      ...
      log_format = join(
        " ",
        [
          for field in local.l1_map[k].log_field_list :
          "$${${field.name}}"
        ]
      )
      log_table_field_list = [
        for field in local.l1_map[k].log_field_list : {
          name = lookup(field, "athena_name", replace(field.name, "-", "_"))
          type = field.type
        }
      ]
