...
variable "group_map" {
  type = map(object({
    ...
    instance_refresh_max_healthy_percentage     = optional(number)
    instance_refresh_min_healthy_percentage     = optional(number)
...
variable "group_instance_lifetime_max_hours_default" {
  type        = number
  default     = 0
...
variable "group_instance_maintenance_max_healthy_percentage_default" {
  ...
  default = 200
  validation {
    condition     = 100 <= var.group_instance_maintenance_max_healthy_percentage_default && var.group_instance_maintenance_max_healthy_percentage_default <= 200
    error_message = "Invalid healthy percentage"
  }
}
...
variable "group_instance_refresh_max_healthy_percentage_default" {
  type    = number
  default = 200
  validation {
    condition     = 100 <= var.group_instance_refresh_max_healthy_percentage_default && var.group_instance_refresh_max_healthy_percentage_default <= 200
    error_message = "Invalid healthy percentage"
  }
}

variable "group_instance_refresh_min_healthy_percentage_default" {
  type    = number
  default = 100
}

variable "group_instance_refresh_protected_instance_enabled_default" {
  type        = bool
  default     = true
